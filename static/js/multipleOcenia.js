...
const randPlacesOceania = ["Australia", "Mirconesia", "Vanuatu", "Tonga", "Palau", "Tuvalu", "Nauru", "Guam", "Solomon Islands", "Marshall Islands", "New Caledonia", "Niue", "Norfolk Island", "Northern Mariana Island", "Pitcairn Islands"]

const chosenArr = []

let randid = Math.floor(Math.random() * randPlacesOceania.length);
let randidOther = Math.floor(Math.random() * randPlacesOceania.length - 1);
...
function randomCountry(input) {
    let includeArr = randPlacesOceania.splice(randid, 1);
    chosenArr.push(input.location);
    console.log(input.location);
    correctAnswer = input.location;

    let newArr = chosenArr.concat(includeArr);
    let includeNew = randPlacesOceania.splice(randidOther, 1);
